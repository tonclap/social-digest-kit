...
Логика: у большинства FB-аккаунтов в переписи нет URL, зато есть имя (ссылки в
списке друзей Facebook не отдаёт скопом). Сверяем имена и дописываем ссылку.
Неоднозначные имена (несколько человек с одинаковым) не трогаем — их разберёт
человек на этапе разметки.
